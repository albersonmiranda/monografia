...
O primeira camada de entendimento em uma pesquisa deste tipo é a exploratória. Após a aplicação das condições mencionadas, a base de dados conta com expressivos `r scales::number(nrow(rais))` de entradas, de 2006 a 2020, e cobre todos os `r length(unique(rais$id_municipio))` municípios do Espírito Santo.
...
```{r count sexo}
#| fig-cap: "Entradas por sexo em 2020"
