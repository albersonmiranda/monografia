...
Seguindo @mincer_schooling_1974, aproximaremos a experiência de cada indivíduos a partir da seguinte fórmula:
...
Assim, supõe-se para um indivíduo de 30 anos com mestrado $30-18-5=7$ anos de experiência.
...
Como a escolaridade, raça/cor e sexo são variáveis qualitativas, cada estado recebe uma variável e coeficiente, exceto os estados definidos como linha de base. Dessa forma, temos:
...
E também serão utilizadas variáveis de interação, que estimam o efeito em pares de estados possíveis (por brevidade, escrevo apenas a interação entre as variáveis `sexo` e `raca_cor`):

- $X_{12}$: sexo = masculino * raça/cor (raca_cor) = amarela
- $X_{13}$: sexo = masculino * raça/cor (raca_cor) = branca
- $X_{14}$: sexo = masculino * raça/cor (raca_cor) = indígena
- $X_{15}$: sexo = masculino * raça/cor (raca_cor) = parda

<!-- TODO: Readequar os dados baseado em Mincer (idade -6) -->
